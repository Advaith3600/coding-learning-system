...
export type ModuleChallengesClientProps = {
  moduleId: ModuleId;
  initialRole: "admin" | "student" | null;
  initialCompleted: Record<string, boolean>;
  initialSolutions: Record<string, string>;
  /** Server supplied progress; skip client fetch on mount. */
  ssrHydrated: boolean;
};

export function ModuleChallengesClient({
  moduleId,
  initialRole,
  initialCompleted,
  initialSolutions,
  ssrHydrated
}: ModuleChallengesClientProps) {
  ...
  const [progressLoaded, setProgressLoaded] = useState(ssrHydrated);
